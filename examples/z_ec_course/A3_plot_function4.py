...
def show_xpos_history(history: list[float]) -> None:
    ...
    # (This variable is for reference but the actual conversion happens later)
    ...
    # Use actual trajectory bounds with some padding
    ...
    # Arena appears to span from spawn (-0.8) to target (5) in X, and roughly [-2, 2] in Y
    ...
    ax.plot(x_img, y_img, 'b-', linewidth=2, alpha=0.8, label='Robot Path')
    
    # Mark start and end points
    ax.plot(x_img[0], y_img[0], 'go', markersize=8, label='Start Position')
    ax.plot(x_img[-1], y_img[-1], 'ro', markersize=8, label='End Position')
    ...
    ax.set_title("Robot Trajectory in Arena")
