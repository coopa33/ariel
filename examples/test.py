...
def get_spawn_position_for_brain_generation(brain_gen_num: int, sim_config: EAConfig) -> list[float]:
    ...
    ### NOTE: have have it return the locations from sim_config
    terrains = [
        [-0.8, 0, 0.1],
        [1.0, 0, 0.1],
        [3, 0, 0.15],
...
def fitness_function_different_spawn(history: list[float], sim_config, brain_gen_number) -> float:
    ...
    xt, yt, zt = sim_config.target_position
    xc, yc, zc = history[-1]
    spawn = get_spawn_position_for_brain_generation(brain_gen_number, sim_config)
    base = sim_config.start_normal
    dx = float(abs(spawn[0] - base[0]))
    dy = float(abs(spawn[1] - base[1]))
    dz = float(abs(spawn[2] - base[2]))
...
def _ensure_sigmas(ind, sigma0: float):
    if not hasattr(ind, 'sigmas'):
        ind.sigmas = [sigma0] * len(ind)

def _calculate_tau(ind_size, prime):
    if prime:
        return 1 / math.sqrt(2 * ind_size)
        
    else:
        return 1 / math.sqrt(2 * math.sqrt(ind_size))
    
def whole_arithmetic_recomb(ind1, ind2, alpha, eps=1e-8):
    """
    ...
    """
    _ensure_sigmas(ind1, sigma0=0.1)
    _ensure_sigmas(ind2, sigma0=0.1)
    # Crossover genotype
...
def mut_self_adapt_gaussian(
    ind,
    indpb: float,
    sigma0: float = 0.1,
    min_sigma: float = 1e-8
    ):
    # Make sure the individual has sigmas
    _ensure_sigmas(ind, sigma0)
    tau_prime = _calculate_tau(len(ind), prime=True)
    tau = _calculate_tau(len(ind), prime=False)
    ### First compute new sigma
    new_sigmas = []
    n0 = random.gauss(0.0, 1.0) # Global noise
    for s in ind.sigmas:
        ni = random.gauss(0.0, 1.0)# Local noise
        new_s = max(s * math.exp(tau_prime * n0 + tau * ni), min_sigma)
        new_sigmas.append(new_s)
    ind.sigmas = new_sigmas
    # Then mutate
    for i in range(len(ind)):
        if random.random() < indpb:
            noise = random.gauss(0.0, ind.sigmas[i])
            ind[i] += noise
...
def EA_brain(robot_graph, ea_brain_config, sim_config, ind_type, mode):
    ...
    toolbox_brain.register(
        "MutateBrain_SelfAdapt",
        mut_self_adapt_gaussian,
        indpb = ea_brain_config.sa_mut_indpb,
        sigma0 = ea_brain_config.sa_mut_sigma0,
        min_sigma = ea_brain_config.sa_mut_min_sigma
...
def EA_body(
    ...
    "MutateBody_SelfAdapt",
    mut_self_adapt_gaussian,
    indpb = ea_body_config.sa_mut_indpb,
    sigma0 = ea_body_config.sa_mut_sigma0,
    min_sigma = ea_body_config.sa_mut_min_sigma
